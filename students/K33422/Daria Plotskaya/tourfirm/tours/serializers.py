...
from .models import Tour, Reservation, Review, ReviewPhoto
...
class TourSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Tour
        fields = (
        "id", "destination", "date_from", "date_to", "hotel", "prev_price",
        "price", "empty_count")
        read_only_fields = (
        "id", "destination", "date_from", "date_to", "hotel", "price",
        "empty_count")
...
class ReservationReadSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Reservation
        fields = (
        "id", "tour", "count", "approved", "total_price", "has_review")
        read_only_fields = (
        "id", "tour", "count", "approved", "total_price", "has_review")
...
class ReservationWriteSerializer(serializers.ModelSerializer):
    tour = serializers.PrimaryKeyRelatedField(write_only=True,
...
class ReviewSerializer(serializers.ModelSerializer):
    # Явно указываем что ID резервирования не должен отображаться при чтении
    # (для анонимности отзывов)
    reservation = serializers.PrimaryKeyRelatedField(write_only=True,
    ...
    class Meta:
        model = Review
        fields = (
        "id", "reservation", "text", "stars", "name", "tour")
    ...
    def get_tour(self, obj):
        return obj.reservation.tour.id

    # Разрешаем оставлять отзыв только на подтвержденое резервирование,
    # только на свое резервирование и только 1 раз
    def validate(self, attrs):
        reservation = attrs["reservation"]
        if not reservation.approved:
            raise serializers.ValidationError(
                "Нельзя оставлять отзыв без подтвержденного резервирования")
        if reservation.has_review:
            raise serializers.ValidationError("Вы уже оставили отзыв")
        # if reservation.user != self.context["request"].user:
        #     raise serializers.ValidationError("Нельзя оставлять отзывы на
        #     чужие резервирования")
        return attrs
...
# Сериализаторы для демонстрации в drf
...
class ReservationAPISerializer(serializers.ModelSerializer):
    class Meta:
        model = Reservation
        fields = "__all__"
